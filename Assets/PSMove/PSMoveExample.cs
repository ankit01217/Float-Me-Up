...
public class PSMoveExample : MonoBehaviour {
	...
	public static string ipAddress = "128.2.239.47"; //RPIS 128.2.239.47 //ALICE 128.2.237.66
	...
	//private MoveController activeMoveController;
	public static Vector3 handleVelocity;
	...
	void FixedUpdate () {
		...
		activeControllerIndex = (PSMoveInput.MoveControllers [0].Connected == true) ? 0 : 1; 
		if(PSMoveInput.IsConnected == true && PSMoveInput.MoveControllers[activeControllerIndex].Connected) {
		
			Vector3 gemPos, handlePos;
			MoveData moveData = PSMoveInput.MoveControllers[activeControllerIndex].Data;
			gemPos = moveData.Position;
			handlePos = moveData.HandlePosition;
			handleVelocity = moveData.HandleVelocity;
			//Debug.Log ("handle velocity :" + PSMoveExample.handleVelocity.z);
